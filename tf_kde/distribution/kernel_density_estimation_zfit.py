...
from tf_kde.helper import hofmeyr as hofmeyr_helper
...
class KernelDensityEstimationFFT(BasePDF):
    ...
    def __init__(self, 
        ...
        self._grid = binning_helper.generate_grid(self._data, num_grid_points=self._num_grid_points)
        self._grid_data = binning_helper.bin(self._binning_method, self._data, self._grid, self._weights)
        self._grid_estimations = convolution_helper.convolve_data_with_kernel(self._kernel, self._bandwidth, self._grid_data, self._grid, self._support, self._fft_method)
...
class KernelDensityEstimationISJ(BasePDF):
    ...
    def __init__(self, 
        ...
        self._bandwidth, self._grid_estimations, self._grid = isj_helper.calculate_bandwidth_and_density(self._data, self._num_grid_points, self._binning_method, self._weights)
...
class KernelDensityEstimationHofmeyr(BasePDF):
    _N_OBS = 1
    ...
    def __init__(self, 
        ...
        if self._implementation == 'numpy':
            self._grid_estimations = hofmeyr_helper.calculate_estimate_numpy(self._grid, self._grid_data, self._betas, self._bandwidth)
        elif self._implementation == 'cpp':
            self._grid_estimations = hofmeyr_helper.calculate_estimate_cpp(self._grid, self._grid_data, self._betas, self._bandwidth)
        else:
            self._grid_estimations = hofmeyr_helper.calculate_estimate(self._grid, self._grid_data, self._betas, self._bandwidth)
